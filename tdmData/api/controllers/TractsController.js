...
lehdTrips : function(req,res){
	...
	Gtfs.query(sql,{},function(err,tracts_data){
		if (err) { res.send('{status:"error",message:"'+err+'"}',500); return console.log(err);}
		var sql = "select O_MAT_LAT as o_lat, O_MAT_LONG as o_lng,D_MAT_LAT as d_lat, D_MAT_LONG as d_lng,o_geoid10,d_geoid10 from survey_geo where o_geoid10 in "+fips_in+" and d_geoid10 in "+fips_in;
